...
public class AddEmployeeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            Map <String, String> errors = ValidationUtil.validate(emp, false);
            if (!errors.isEmpty()) {
                // send errors back to add.jsp
                DepartmentDAO deptDAO = new DepartmentDAO();
                List<Department> depts = deptDAO.getAll();
                request.setAttribute("depts", depts);
            ...
            if (saved) {
                ...
                logDAO.log("ADD", "Employee " + emp.getName() + " was added", emp.getName());
                // ── Toast success ─────────────────
                request.getSession().setAttribute("toast_msg", emp.getName() + " added successfully");
                request.getSession().setAttribute("toast_type", "success");
                response.sendRedirect("employees");
            } else {
                // ── Toast error ───────────────────
                request.getSession().setAttribute("toast_msg", "Failed to add employee. Try again.");
